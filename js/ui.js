...
export default class UI {
  ...
  drawCardButton(ctx, button, isHovered, isClicked, alpha = 1) {
    ...
    if (isWideCard) {
      ...
      ctx.fillStyle = button.iconBg || '#EDEAE6';
  ...
  _drawIcon(ctx, icon, iconCenterX, iconCenterY, isMobile, iconColor) {
    ...
    if (icon === 'book') {
      ...
      ctx.strokeStyle = iconColor || '#5BA88F';
      // 袋身
      ctx.beginPath();
  ...
  drawModeSwitcher(ctx, x, y, width, height) {
    ...
    ctx.fillStyle = isTimedActive ? '#FFFFFF' : '#6B6878';
  ...
  drawScoreHistoryTabs(ctx, modalX, modalY, modalWidth, isMobile) {
    ...
    tabs.forEach((tab, index) => {
      ...
      let bgColor = isActive ? '#5BA88F' : scheme.cardBg;
  ...
  renderHighScoreCelebration(ctx) {
    ...
    ctx.shadowBlur = 30;
    this.drawBrutalismRect(ctx, cardX, cardY, cardWidth, cardHeight, '#F5F3F0', {
      shadowOffset: 8,
      borderWidth: 4,
      radius: 24
    });
  ...
  createComboParticles(level, count) {
    ...
    const color = level ? level.color : '#5BA88F';
  ...
  renderShopProducts(ctx, modalX, modalY, modalWidth, modalHeight, isMobile) {
    ...
    products.forEach((product, index) => {
      ...
      if (canBuy) {
        ...
        this.drawBrutalismRect(ctx, buyButtonX, buyButtonY, buyButtonWidth, buyButtonHeight, '#6B6878', {
          shadowOffset: 2,
          borderWidth: 1
        });
  ...
  renderSkillsCategories(ctx, modalX, modalY, modalWidth, modalHeight, isMobile) {
    ...
    for (const [category, skills] of this.skillsData) {
      ...
      for (const skill of skills) {
        ...
        ctx.fillStyle = skill.isUnlocked ? '#5BA88F' : scheme.text;
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
        ...
        if (skill.isUnlocked) {
          ...
          this.drawBrutalismRect(ctx, unlockButtonX, unlockButtonY, unlockButtonWidth, unlockButtonHeight, '#6B6878', {
            shadowOffset: 2,
            borderWidth: 1
          });
